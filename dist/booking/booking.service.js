...
import { BookingStatus, PaymentMethod, PaymentStatus, MembershipStatus, MembershipType, } from "@prisma/client";
...
class BookingService {
     ...
     */
    validateFutureSessionDate(sessionDate) {
        ...
        if (selectedDate < tomorrow) {
            throw new Error("You can only select sessions from tomorrow onwards.");
        }
        return true;
    }
    /**
     ...
     */
    hasRemainingCredit(memberMembership) {
        // Unlimited
        if (memberMembership.creditsTotal ===
            null) {
            return true;
        }
        return (memberMembership.creditsUsed <
    ...
    async createBooking(data) {
        ...
        if (membership.type ===
            ...
            !classSessionId) {
            throw new Error("Please select an available class session.");
        }
        ...
        if (classSessionId) {
            ...
            if (!classSession.schedule.isActive) {
                throw new Error("This class schedule is no longer active.");
            }
            if (classSession.status !==
                "OPEN") {
            ...
            // TOMORROW ONWARD
            // -----------------------------------------------------
            this.validateFutureSessionDate(classSession.sessionDate);
            // -----------------------------------------------------
            // CLASS MATCH
            // -----------------------------------------------------
            if (data.classId &&
                ...
                throw new Error("The selected session does not belong to the selected class.");
            }
            // -----------------------------------------------------
            // CAPACITY
            // -----------------------------------------------------
            ...
            const bookedCount = classSession._count.bookings;
            if (bookedCount >= capacity) {
                throw new Error("This class session is already full.");
            }
        }
        // -------------------------------------------------------
        // HEALTH DECLARATION
        // -------------------------------------------------------
        const healthDeclaration = data.healthDeclaration;
        if (!healthDeclaration ||
            healthDeclaration.accepted !== true) {
            throw new Error("You must accept the Health Declaration before payment.");
        }
        // -------------------------------------------------------
        // PAYMENT REFERENCE
        // -------------------------------------------------------
        const paymentReference = `SL-${Date.now()}-${crypto
            .randomBytes(4)
            .toString("hex")}`;
        // -------------------------------------------------------
        // FLOW TOKEN
        ...
        const booking = await prisma.$transaction(async (tx) => {
            ...
            if (classSession) {
              ...
              FOR UPDATE
            `;
                const latestSession = await tx.classSession.findUnique({
                    where: {
                        id: classSession.id,
                    },
                    include: {
                ...
                this.validateFutureSessionDate(latestSession.sessionDate);
                const capacity = latestSession.capacity ??
                    latestSession.schedule.capacity;
                if (latestSession._count.bookings >=
                    capacity) {
                    throw new Error("This class session has just become full. Please select another session.");
                }
            }
            const createdBooking = await tx.booking.create({
                data: {
                    ...
                    scheduleId: classSession?.scheduleId ??
                        null,
                    bookingDate: classSession?.sessionDate ??
                        null,
                    preferredStartDate: classSession?.sessionDate ??
                        null,
     ...
     */
    async initializePayment(bookingId, bookingFlowToken) {
    ...
    // =========================================================
    async continueGuestBooking(reference) {
        if (!reference ||
            typeof reference !==
                "string") {
            throw new Error("Payment reference is required.");
        ...
        if (booking.bookingStatus !==
            ...
            throw new Error("This booking is no longer available to continue.");
        }
        if (booking.userId) {
            throw new Error("This booking is already attached to an account.");
        }
        const { token: bookingFlowToken, tokenHash, } = this.generateBookingFlowToken();
     ...
     */
    async getClassAvailability(classId, fromDate, toDate) {
        if (!classId) {
        ...
        // START DATE
        // -------------------------------------------------------
        let startDate = this.getTomorrowStart();
        if (fromDate) {
            const requestedStart = new Date(fromDate);
            if (Number.isNaN(requestedStart.getTime())) {
                throw new Error("Invalid from date.");
            }
            this.validateFutureSessionDate(requestedStart);
        ...
        return sessions.map((session) => {
            ...
            const availableSlots = Math.max(capacity -
                bookedCount, 0);
            return {
                id: session.id,
    ...
    // =========================================================
    async getSessionAvailability(sessionId) {
        ...
        this.validateFutureSessionDate(session.sessionDate);
        const capacity = session.capacity ??
            session.schedule.capacity;
        const bookedCount = session._count.bookings;
        const availableSlots = Math.max(capacity -
    ...
    async confirmBooking(bookingId, userId) {
        ...
        if (booking.membership.type ===
            MembershipType.GROUP) {
            return await this.confirmGroupBooking(bookingId, userId);
        }
        // =======================================================
        // PRIVATE
        // =======================================================
        return await this.confirmPrivateBooking(bookingId, userId);
    }
    // =========================================================
    // CONFIRM GROUP BOOKING
    // =========================================================
    async confirmGroupBooking(bookingId, userId) {
        const confirmedBooking = await prisma.$transaction(async (tx) => {
          ...
          `;
            ...
            const session = currentBooking.session;
            if (!session) {
                throw new Error("No class session has been selected.");
            }
            // -------------------------------------------------
          ...
          `;
            ...
            // MEMBERSHIP EXPIRY
            // -------------------------------------------------
            const membershipStart = session.sessionDate;
            const membershipExpiry = this.calculateMembershipExpiry(membershipStart, currentBooking.membership
                .duration, currentBooking.membership
                .period);
            // -------------------------------------------------
            // CREDITS
            // -------------------------------------------------
            const creditsTotal = this.getMembershipCredits(currentBooking.membership
                .classLimit);
            // -------------------------------------------------
            // CREATE MEMBERSHIP
            // -------------------------------------------------
            ...
            // CHECK CREDIT
            // -------------------------------------------------
            if (!this.hasRemainingCredit(memberMembership)) {
                throw new Error("You do not have any remaining class credits.");
            }
            // -------------------------------------------------
    ...
    // =========================================================
    async confirmPrivateBooking(bookingId, userId) {
        return await prisma.$transaction(async (tx) => {
            await tx.$queryRaw `
          SELECT id
          FROM "Booking"
        ...
        `;
            ...
            const creditsTotal = this.getMembershipCredits(booking.membership
                .classLimit);
            const membershipStart = booking.preferredStartDate ??
                new Date();
            const membershipExpiry = this.calculateMembershipExpiry(membershipStart, booking.membership
                .duration, booking.membership
                .period);
            ...
            if (!this.hasRemainingCredit(memberMembership)) {
                throw new Error("You do not have any remaining session credits.");
            }
            if (memberMembership
                .creditsTotal !==
                null) {
            ...
            return await tx.booking.update({
                ...
                data: {
                    bookingStatus: BookingStatus.CONFIRMED,
                    preferredStartDate: membershipStart,
                },
                include: {
                    membership: true,
                    memberMembership: true,
    ...
    async bookMemberSession(userId, sessionId) {
        ...
        `;
            ...
            const capacity = session.capacity ??
                session.schedule.capacity;
            if (session._count.bookings >=
                capacity) {
                throw new Error("This class session is full.");
            }
            // -----------------------------------------------------
            ...
            if (membership.expiryDate &&
                ...
                throw new Error("This session is outside your membership validity period.");
            }
            // -----------------------------------------------------
            // CHECK CREDIT
            // -----------------------------------------------------
            if (!this.hasRemainingCredit(membership)) {
                throw new Error("You do not have any remaining class credits.");
            }
            // -----------------------------------------------------
    ...
    // =========================================================
    async getMembershipCreditSummary(userId) {
        ...
        const remainingCredits = membership.creditsTotal ===
            null
            ? null
            : Math.max(membership.creditsTotal -
                membership.creditsUsed, 0);
        return {
            memberMembershipId: membership.id,
            membership: membership.membership,
            creditsTotal: membership.creditsTotal,
            creditsUsed: membership.creditsUsed,
            remainingCredits,
            unlimited: membership.creditsTotal ===
                null,
        ...
        return await prisma.healthSafetyForm.upsert({
            ...
            create: {
                ...
                dueDate: data.dueDate
                    ...
                    : null,
                pregnancyClearance: data.pregnancyClearance ||
                    null,
                ...
                deliveryDate: data.deliveryDate
                    ...
                    : null,
                postpartumClearance: data.postpartumClearance ||
                    null,
                ...
                surgeryDetails: data.surgeryDetails ||
                    null,
                surgeryClearance: data.surgeryClearance ||
                    null,
    ...
    `;
        ...
        if (result.calendarEventId) {
            try {
                ...
                console.error(`⚠️ Failed to remove Google Calendar event for cancelled booking ${bookingId}:`, calendarError);
            }
        }
        return result;
    }
