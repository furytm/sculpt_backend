...
class AdminSessionController {
        ...
        try {
            ...
            const sessions = await adminSessionService.getSessions({
                ...
                status: typeof status === "string" &&
                    ["OPEN", "FULL", "CANCELLED", "COMPLETED"].includes(status)
                    ? status
                    : undefined,
    ...
    async getSession(req, res) {
        try {
            ...
            const session = await adminSessionService.getSessionById(String(sessionId));
            return res.status(200).json({
                success: true,
                data: session,
            });
    ...
    async cancelSession(req, res) {
        try {
            ...
            const result = await adminSessionService.cancelSession(sessionId);
            return res.status(200).json({
                success: true,
                ...result,
            });
            ...
            console.error("Cancel admin session error:", error);
            return res.status(400).json({
                success: false,
                message: error?.message ||
    ...
    async reopenSession(req, res) {
        try {
            ...
            const result = await adminSessionService.reopenSession(sessionId);
            return res.status(200).json({
                success: true,
                ...result,
            });
            ...
            console.error("Reopen admin session error:", error);
            return res.status(400).json({
                success: false,
                message: error?.message ||
