...
class ClassSessionService {
  ...
  async generateFutureSessions(daysAhead = GENERATION_DAYS) {
    ...
    for (const schedule of schedules) {
      ...
      if (targetDay === undefined) {
        console.warn(
          `⚠️ Unknown day of week for schedule ${schedule.id}: ${schedule.dayOfWeek}`
        );
        continue;
      }
      ...
      for (let offset = 0; offset < daysAhead; offset++) {
        ...
        const sessionDate = this.createSessionDate(
          calendarDate,
          schedule.startTime
        );
        ...
        console.log(
          `✅ Created session: ${schedule.className} | ${schedule.dayOfWeek} | ${schedule.startTime} | ${this.formatDate(
            sessionDate
          )}`
        );
      }
    ...
    console.log(
      `📅 Class session generation complete. Created: ${sessionsCreated}, Existing: ${sessionsExisting}`
    );
  ...
  private parseTime(time: string): {
    ...
    throw new Error(
      `Invalid schedule time format: ${time}`
    );
  }
